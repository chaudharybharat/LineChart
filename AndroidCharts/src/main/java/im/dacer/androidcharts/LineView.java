...
public class LineView extends View {
    ...
    private final int MIN_VERTICAL_GRID_NUM = 4;private final int MIN_HORIZONTAL_GRID_NUM = 1;
        ...
        for (int k = 0; k < drawDotLists.size(); k++) {
            ...
            for (int i = 0; i < drawDotLists.get(k).size() - 1; i++) {
              canvas.drawLine(drawDotLists.get(k).get(i).x, drawDotLists.get(k).get(i).y,
                ...
                RectF rectF = new RectF(drawDotLists.get(k).get(i).x,drawDotLists.get(k).get(i).y, drawDotLists.get(k).get(i + 1).x, drawDotLists.get(k).get(i + 1).y);
